...
if ! command -v "${LIS_JAVA:-java}" >/dev/null 2>&1; then
...
if [ ! -d libraries ]; then
    ...
    "${LIS_JAVA:-java}" -jar "$INSTALLER" -installServer
fi

if [ ! -e server.properties ]; then
    printf "allow-flight=true\nmotd=Lissencéphapack\nmax-tick-time=180000" > server.properties
fi


JAVA_VERSION=$("${LIS_JAVA:-java}" -fullversion 2>&1 | awk -F '"' '/version/ {print $2}' | cut -d'.' -f1)
...
while true
do
    "${LIS_JAVA:-java}" @user_jvm_args.txt @libraries/net/minecraftforge/forge/1.18.2-$FORGE_VERSION/unix_args.txt nogui

    if [ "${LIS_RESTART:-true}" = "false" ]; then
        exit 0
    fi
